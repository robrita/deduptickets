...
class Ticket(BaseModel):
    ...
    content_vector: list[float] | None = Field(default=None, description="Embedding vector")
    dedup_text: str | None = Field(default=None, description="Concatenated text used for embedding")
    dedup: dict[str, Any] | None = Field(
        default=None, description="Dedup decision metadata (score, decision, signals)"
    )
    ...
    @staticmethod
    def generate_partition_key(created_at: datetime) -> str:
        """Generate partition key from created_at date."""
